...
export async function getNextTopicalComment(
  ...
  try {
    ...
    // Use the shared comments query to apply moderation filters
    const commentQueryParams: Partial<GetCommentsParams> = {
      zid,
      tids,
      withoutTids,
      random: true,
    ...
    const rows = await getComments(commentQueryParams as GetCommentsParams);
    ...
    // Fetch and include remaining/total counts
    const remainingRows = await getNumberOfCommentsRemaining(zid, pid);
    if (!remainingRows || remainingRows.length === 0) {
      throw new Error(`polis_err_getNumberOfCommentsRemaining_${zid}_${pid}`);
    }
    ...
    comment.remaining = remainingCount;
