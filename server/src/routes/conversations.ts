...
function handle_PUT_conversations(
  req: {
    p: {
      ...
      internal_information_submitted?: boolean;
      seed_comments_submitted?: boolean;
      use_xid_whitelist?: any;
    };
  },
  res: any
) {
  console.log('🚨 handle_PUT_conversations called!');
  ...
  isModerator(req.p.zid, req.p.uid)
    .then(function (ok: any) {
      ...
      console.log('🔧 PUT conversations - internal_information in req.p:', req.p.internal_information);
      ...
      // Persist conversation context (used for questionnaire/setup data).
      // Only update if the client actually sent a context field so we don't
      // accidentally clear context when it's omitted.
      const hasContextField =
        (req as any).body && Object.prototype.hasOwnProperty.call((req as any).body, "context");
      if (hasContextField) {
