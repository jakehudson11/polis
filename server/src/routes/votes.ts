...
async function handle_POST_votes(req: RequestWithP, res: any) {
  ...
  try {
    ...
    // 4. Wait briefly for the database RULE to update votes_latest_unique
    // This ensures the vote is reflected in queries that filter by not_voted_by_pid
    await new Promise(resolve => setTimeout(resolve, 50));

    // 5. Get next comment (excluding the one just voted on)
    const nextComment = await getNextComment(zid, pid, [tid], lang);
