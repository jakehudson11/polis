...
import { handle_POST_delphi_jobs, handle_GET_delphi_jobs, handle_GET_delphi_job } from "./src/routes/delphi/jobs";
...
helpersInitialized.then(
  function (o: any) {
    ...
    app.post("/api/v3/auth/forgot-password", moveToBody, handle_POST_auth_forgot_password);

    app.post("/api/v3/auth/reset-password", moveToBody, handle_POST_auth_reset_password);
    ...
    // Add GET endpoint for a single Delphi job by ID
    app.get(
      "/api/v3/delphi/jobs/:jobId",
      moveToBody,
      hybridAuth(assignToP),
      function (req, res) {
        try {
          handle_GET_delphi_job(req, res);
        } catch (err) {
          res.json({
            status: "error",
            message: "Internal server error in single job endpoint",
    ...
    app.put(
      ...
      want("context", getOptionalStringLimitLength(50000), assignToP),
