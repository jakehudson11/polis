...
async function _handleJWTConversationMismatch(
  ...
  if (req.p.emailPasswordUser) {
    // Email/password users should maintain their identity
    // but get a new participant record for the new conversation
    req.p.pid = undefined;
    return true;
  }
...
async function _ensureParticipantInternal(
  ...
  console.log("🔍 [ensureParticipant] Before _getOrCreateParticipant:", {
    pid,
    uid,
    zid,
    createIfMissing,
  ...
  if ((createIfMissing || req.p.xid) && uid !== undefined && (pid === undefined || pid === -1)) {
    // Create participants for:
    // 1. Normal cases when createIfMissing=true and pid is not set
