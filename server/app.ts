...
import { handle_GET_delphi_jobs } from "./src/routes/delphi/jobs";
...
import { handle_POST_generate_seed_comments, handle_POST_submit_seed_comments } from "./src/routes/seedComments";
import { handle_POST_generate_guiding_questions } from "./src/routes/guidingQuestions";
import {
  handle_GET_math_pca,
...
helpersInitialized.then(
  function (o: any) {
    ...
    app.use(express.bodyParser({ limit: '10mb' })); // Increased limit to handle large questionnaire responses
    ...
    app.post("/api/v3/auth/register", moveToBody, handle_POST_auth_register);

    app.post("/api/v3/auth/login", moveToBody, handle_POST_auth_login);
    ...
    app.get("/api/v3/delphi/jobs", moveToBody, hybridAuth(assignToP), handle_GET_delphi_jobs);
    ...
    // Generate seed comments from questionnaire
    app.post(
      "/api/v3/conversations/:conversation_id/generate-seed-comments",
      moveToBody,
      hybridAuth(assignToP),
      need(
        "conversation_id",
      ...
      handle_POST_generate_seed_comments
    );

    // Submit seed comments
    app.post(
      "/api/v3/conversations/:conversation_id/submit-seed-comments",
      moveToBody,
      hybridAuth(assignToP),
      need(
        "conversation_id",
      ...
      handle_POST_submit_seed_comments
    );

    // Generate guiding questions from questionnaire
    app.post(
      "/api/v3/conversations/:conversation_id/generate-guiding-questions",
      moveToBody,
      hybridAuth(assignToP),
      need(
        "conversation_id",
      ...
      handle_POST_generate_guiding_questions
    );
    ...
    app.get(
      ...
      want("context", getStringLimitLength(1, 100000), assignToP),
      want("xid", getStringLimitLength(1, 999), assignToP),
      handle_GET_conversations
    );
    ...
    app.get(/^\/register(\/.*)?/, fetchIndexForAdminPage);
    app.get(
