...
const Conversations = () => {
  ...
  const getUserType = () => {
    ...
    if (userState?.user) return 'User'
    return 'Guest'
  }

  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: [3, null, 4] }}>
        <Heading
          as="h3"
          sx={{
            fontSize: [3, null, 4],
        ...
        <Text sx={{ fontSize: [1, 2], color: 'textSecondary', fontWeight: 'medium' }}>
          {getUserType()}
        </Text>
      </Box>
      {interstitialVisible && (
          ...
          <Box>
            ...
            {(allConversations || []).map((c) => (
              <Box
                ...
                <Box>
                  <Text sx={{ display: ['inline', 'none'], fontWeight: 'bold' }}>Name: </Text>
                  {c.topic}
                </Box>
